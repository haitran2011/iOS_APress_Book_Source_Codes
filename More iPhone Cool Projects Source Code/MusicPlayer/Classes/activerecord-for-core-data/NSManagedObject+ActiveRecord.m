...
+ (NSArray *)propertiesNamed:(NSArray *)properties
{
	...
	if (properties)
	{
		...
		for (NSString *propertyName in properties)
		{
			...
			{
				NSLog(@"Property '%@' not found in %d properties for %@", propertyName, [propDict count], NSStringFromClass(self));
			}
		}
	}
...
+ (NSArray *) sortAscending:(BOOL)ascending attributes:(id)attributesToSortBy, ...
{
	...
	if ([attributesToSortBy isKindOfClass:[NSArray class]])
	{
		...
		while (attributeName = va_arg(variadicArguments, id))
		{
...
+ (id)newEntity
{	
	NSManagedObject *newEntity = [self newInContext:[NSManagedObjectContext defaultContext]];
