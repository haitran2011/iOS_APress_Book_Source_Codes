...
- (void)viewWillAppear:(BOOL)animated {
	...
	cubeIt = ^(int num)	{ return num * num * num;  };
	
	NSLog(@"The cube of %d is %d", numberToCube, cubeIt(numberToCube));
	
	int (^cubeItTimesAFactor)(int) = ^(int num) {return cubeIt(num)*mFactor;  };
	
	NSLog(@"The cube of %d multiplied by a factor of %d is %d", numberToCube, mFactor, cubeItTimesAFactor(4));
...
-(UIImage *)getImageForURL:(NSURL *)url
{
	...
	if (object == nil) {
		...
		PLBlockOperation *fetchImageOp = [PLBlockOperation blockOperationWithBlock:^{
			NSAutoreleasePool *localPool;
			@try {
				localPool = [[NSAutoreleasePool alloc] init];
				// fetch the image
				...
				NSDictionary *result = [[NSDictionary alloc] initWithObjectsAndKeys:image,@"image",url,@"url",nil];
				
				
				[self performSelectorOnMainThread:@selector(storeImageForURL:) withObject:result waitUntilDone:NO];
			...
			@catch (NSException * e) {
				NSLog(@"error in block operation");
			}
			@finally {
				[localPool release];
			}
